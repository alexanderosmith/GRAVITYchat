...
# Dotenv credentials for Zotero that are in the gitignore: needs configuration
...
for item in items:
    ...
    articleID = item["data"][
        "key"
    ]  # set up an attribute of the Zotero internal key ("ID")
                        ...
                        if each["data"]["contentType"] == "application/pdf":
                            ...
                            for pg in range(0, totalPages, step):
                                content += extract_text(
                                    "zot_article.pdf",
                                    page_numbers=list(range(pg, pg + step)),
                                    laparams=laparam,
                                )
                            ...
                            if len(content) < 200:
                                ...
                                ## Output to file:
                                with open(
                                    os.path.join("texts", f"{articleID}.txt"),
                                    "w",
                                    encoding="utf-8",
                                ) as f:
                                    f.write(content)
...
for item in items:
    ...
    currentArticle = {
        ...
        "journal": content["publicationTitle"]
        if not content["itemType"].startswith("book")
        else "",
        "booktitle": content["publicationTitle"]
        if content["itemType"].startswith("book")
        else "",
